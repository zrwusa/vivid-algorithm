...
export function canJumpMy(nums: number[]): boolean {
  const zeros: Map<number, boolean> = new Map();
  for (let i = 0; i < nums.length; i++) {
    for (const [, can] of zeros) {
      if (!can) {
        return false;
      }
    }
...
export function twoSumByArray(nums: number[], target: number): number[] {
