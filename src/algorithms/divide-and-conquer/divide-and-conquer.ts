...
export const majorityElement = function (nums: number[]) {
  let majority = nums[0];
  let count = 0;
...
export const findMin = function (nums: number[]): number {
  const n = nums.length;
  const first = nums[0];
  if (n === 1) {
    return first;
...
export const countSmallerNoReverse = function (nums: number[]): number[] {
...
export const countSmallerBITPlagiarized = (nums: number[]): number[] => {
...
export const runAllCountSmaller = async () => {
...
export function kthSmallestDC(matrix: number[][], k: number): number {
  let arr: number[] = [];
