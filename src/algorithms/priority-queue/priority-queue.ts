...
export function findKthLargestMinHeap(nums: number[], k: number): number {
  const heap = new PriorityQueue<number>([], {comparator: (a, b) => a - b});
  for (const i of nums) {
...
function mergeKLists(lists: DoublyLinkedListNode[]): DoublyLinkedListNode | undefined {
  // TODO dev tools was disconnected issue
  const heap = new PriorityQueue<DoublyLinkedListNode>([], {comparator: (a, b) => a.value - b.value});
  ...
  const ans: DoublyLinkedListNode | undefined = polled ? polled : undefined;
  if (ans) {
    ans.prev = undefined;
    if (ans.next) {
      heap.add(ans.next);
    ...
    while (!heap.isEmpty()) {
      // TODO after no-non-null-assertion not ensure the logic
      const polled = heap.poll();
      const cur = polled ? polled : undefined;
      if (cur) {
        cur.prev = prev;
        prev.next = cur;
        prev = prev.next;
...
function topKFrequent(nums: number[], k: number): number[] {
  ...
  const minHeap = new PriorityQueue<[number, number]>([], {comparator: (a, b) => a[1] - b[1]});
...
function reorganizeString(s: string): string {
  ...
  // TODO after no-non-undefined-assertion not ensure the logic
  const peek = heap.peek();
  const peekVal = peek ? peek : undefined;
  if (peek && peekVal) {
...
class KthLargest {
  ...
  constructor(k: number, nums: number[]) {
    this._k = k;
    this._heap = new PriorityQueue<number>(nums, {comparator: (a, b) => a - b});
...
const testPriorityQueue1 = () => {
  const minPriorityQueue = new PriorityQueue<number>([5, 2, 3, 4, 6, 1], {comparator: (a, b) => a - b});
  console.log(minPriorityQueue.toArray(), _.isEqual(minPriorityQueue.toArray(), [1, 4, 2, 5, 6, 3]));
  minPriorityQueue.poll();
  minPriorityQueue.poll();
  minPriorityQueue.poll();
  console.log(minPriorityQueue.toArray(), _.isEqual(minPriorityQueue.toArray(), [4, 5, 6]));
...
const testPriorityQueue2 = () => {
  const maxPriorityQueue = new PriorityQueue<number>([5, 2, 3, 4, 6, 1], {comparator: (a, b) => b - a});
  console.log(maxPriorityQueue.toArray(), _.isEqual(maxPriorityQueue.toArray(), [6, 5, 3, 2, 4, 1]));
  maxPriorityQueue.poll();
  maxPriorityQueue.poll();
  maxPriorityQueue.poll();
  console.log(maxPriorityQueue.toArray(), _.isEqual(maxPriorityQueue.toArray(), [3, 2, 1]));
...
const testPriorityQueue3 = () => {
  const heap = new PriorityQueue<number>([2, 5, 8, 3, 1, 6, 7, 4], {comparator: (a, b) => a - b});
  const clonedPriorityQueue = heap.clone();
  console.log(_.isEqual(clonedPriorityQueue.elements, heap.elements));
  console.log(_.isEqual(clonedPriorityQueue.sort(), [1, 2, 3, 4, 5, 6, 7, 8]));
  console.log(heap.dfs('IN'), _.isEqual(heap.dfs('IN'), [5, 4, 2, 3, 1, 8, 6, 7]));
  console.log(heap.dfs('POST'), _.isEqual(heap.dfs('POST'), [5, 4, 3, 2, 8, 7, 6, 1]));
  console.log(heap.dfs('PRE'), _.isEqual(heap.dfs('PRE'), [1, 2, 4, 5, 3, 6, 8, 7]));
};

const heapSort = async () => {
  const values = Array.from(new Array(10000), () => getRandomInt(1, 10000000));
  const minPriorityQueue = new PriorityQueue<number>(values, {comparator: (a, b) => a - b});
...
export const testPriorityQueue = () => {
  const minPriorityQueue = new PriorityQueue([3, 2, 4, 5, 1, 9], {comparator: (a, b) => a - b});
  console.log(minPriorityQueue.sort());
  console.log(minPriorityQueue.sort());
  const maxPriorityQueue = new PriorityQueue([3, 2, 4, 5, 1, 9], {comparator: (a, b) => b - a});
...
export function kthSmallestInSortedMatrix(matrix: number[][], k: number): number {
  ...
  minHeap.add({value: matrix[0][0], y: 0, x: 0});
  ...
  while (--k > 0) {
    ...
    if (y < matrix.length - 1 && matrix[y + 1][x] !== Infinity) {
      minHeap.add({value: matrix[y + 1][x], y: y + 1, x});
      matrix[y + 1][x] = Infinity;
    }

    if (x < matrix[0].length - 1 && matrix[y][x + 1] !== Infinity) {
      minHeap.add({value: matrix[y][x + 1], y, x: x + 1});
      matrix[y][x + 1] = Infinity;
    }
  }

  return minHeap.peek()?.value || 0;
}
